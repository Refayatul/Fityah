...
Fityah exposes an API that lets other apps control it, much like Shizuku does: your app binds to a
...
- **Transport:** an exported AIDL bound service (`IFityahApi`).
- **Identity:** the calling app's verified UID, resolved to its package. No tokens.
- **Permission:** the user approves your app from a dialog Fityah shows. They can remove access any
  time under **Reducers → Fityah API → Connected apps**, or turn the whole API off with one switch.
...
Copy [`IFityahApi.aidl`](app/src/main/aidl/com/refayatul/fityah/api/IFityahApi.aidl) into your project,
keeping the exact path and package:

```
src/main/aidl/com/refayatul/fityah/api/IFityahApi.aidl
```
...
On Android 11 (API 30) and above you must declare that you talk to Fityah, or you will not be able
to see or bind the service. Add this to your `AndroidManifest.xml`:

```xml
<queries>
    <intent>
        <action android:name="com.refayatul.fityah.api.BIND" />
    </intent>
    <intent>
        <action android:name="com.refayatul.fityah.api.REQUEST_PERMISSION" />
    </intent>
</queries>
```
...
| Purpose | Action |
| --- | --- |
| Bind the service | `com.refayatul.fityah.api.BIND` |
| Ask the user for permission | `com.refayatul.fityah.api.REQUEST_PERMISSION` |

> **Find Fityah by action, not by a hardcoded package.** Debug builds of Fityah use the application
> id `com.refayatul.fityah.debug`, release builds use `com.refayatul.fityah`. Resolve the package at
...
```kotlin
private var api: IFityahApi? = null

private val connection = object : ServiceConnection {
    override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
        api = IFityahApi.Stub.asInterface(service)
    }
...
fun connect() {
    val resolved = packageManager.resolveService(Intent("com.refayatul.fityah.api.BIND"), 0)
        ?: return // Fityah not installed
    val fityahPackage = resolved.serviceInfo.packageName
    val intent = Intent("com.refayatul.fityah.api.BIND").setPackage(fityahPackage)
...
**for result** so Fityah can read your package reliably:
...
fun requestPermission(fityahPackage: String) {
    val intent = Intent("com.refayatul.fityah.api.REQUEST_PERMISSION").setPackage(fityahPackage)
...
> Binder calls run on a pool thread and may block briefly while Fityah writes its settings. Call
...
// 2. if (api?.isGranted != true) requestPermission(fityahPackage)
// 3. once granted:
...
- **Effects vs writes.** Commands always write Fityah's settings, but a visible effect (focus mode
  actually blocking, DND flipping) only happens while Fityah's accessibility services are enabled.
...
adb shell am start -n com.refayatul.fityah.apitester/.MainActivity
```
