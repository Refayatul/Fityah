package com.refayatul.fityah.blockers.uihider.script
...
class ScriptLanguageTest {
    ...
    @Test fun shippedDefaultScriptsAllParse() {
        for (script in com.refayatul.fityah.hardcoded.DEFAULT_UIHIDER_SCRIPTS) {
            try {
    ...
    @Test fun shippedReelDetectorScriptsAllParse() {
        for ((packageName, data) in com.refayatul.fityah.hardcoded.ReelAppConfig.reelData) {
            try {
    ...
    @Test fun scriptStorePersistsToDisk() {
        ...
        try {
            val store = com.refayatul.fityah.blockers.uihider.ScriptStore(file)
            store.put("s1", "name", "hello")
            store.put("s1", "nums", listOf(1.0, 2.0, 3.0))
            store.put("s2", "flag", true)
            store.close()  // flushes synchronously

            val reopened = com.refayatul.fityah.blockers.uihider.ScriptStore(file)
