package com.refayatul.fityah.ui.fragments.main.usage
...
import com.refayatul.fityah.ui.views.WeeklyBarGraphView
import com.refayatul.fityah.utils.UsageStatsHelper
import com.refayatul.fityah.utils.getDefaultLauncherPackageName
...
import com.refayatul.fityah.data.db.ReelUsageStatsEntity
...
class AllAppsUsageViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun refreshSyncedUsage() {
        val provider = com.refayatul.fityah.data.sync.SyncGateway.provider
    ...
    private suspend fun loadDayStats(date: LocalDate) {
        ...
        val dateString = com.refayatul.fityah.utils.TimeTools.dayKey(date)
        ...
        if (remote.isNotEmpty()) {
            val syncedWebsites = remote.map { (domain, ms) ->
                WebsiteStatsEntity(
                    date = dateString,
                    packageName = com.refayatul.fityah.data.sync.SYNCED_WEB_PACKAGE,
            ...
            statsOut = (appStats + AllAppsUsageFragment.Stat(
                com.refayatul.fityah.data.sync.SYNCED_WEB_PACKAGE,
                remote.values.sum(),
    ...
    private suspend fun remoteAppTotals(date: LocalDate): Map<String, Long> = runCatching {
        com.refayatul.fityah.data.sync.SyncGateway.provider.remoteAppUsage(date.toString())
    }.getOrDefault(emptyMap())

    private suspend fun remoteWebsiteTotals(date: LocalDate): Map<String, Long> = runCatching {
        com.refayatul.fityah.data.sync.SyncGateway.provider.remoteWebsiteUsage(date.toString())
    }.getOrDefault(emptyMap())
    ...
    fun getAppMetadata(packageName: String): AppMetadata {
        if (packageName == com.refayatul.fityah.data.sync.SYNCED_WEB_PACKAGE) {
            return AppMetadata(
                label = getApplication<android.app.Application>().getString(com.refayatul.fityah.R.string.synced_browsing),
                category = getApplication<android.app.Application>().getString(com.refayatul.fityah.R.string.synced_other_devices),
                isSystemApp = false,
                installDate = "",
                lastUpdate = "",
                icon = androidx.core.content.ContextCompat.getDrawable(getApplication(), com.refayatul.fityah.R.drawable.ic_synced_web),
            )
        }
