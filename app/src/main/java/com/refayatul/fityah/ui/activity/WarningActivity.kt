...
import com.refayatul.fityah.Constants
import com.refayatul.fityah.nfc.NfcUnlockUtils
...
import com.refayatul.fityah.databinding.DialogWarningOverlayBinding
import com.refayatul.fityah.utils.DataStoreManager
import com.refayatul.fityah.utils.FocusGoalProgress
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
                ...
                object : CountDownTimer(warningScreenConfig.proceedDelayInSecs * 1000L, 1000) {
                    ...
                    override fun onFinish() {
                        binding.btnProceed.let { button ->
                            ...
                            } else if (warningScreenConfig.isTrustedContactUnlockEnabled) {
                                binding.trustedContactContainer.visibility = View.VISIBLE
                                setPrimaryUnlockActionReady(false)
                                button.setText(R.string.proceed)
                                
                                val settings = runBlocking { DataStoreManager(applicationContext).settings.first() }
                                val pinHash = settings.guardianPinHash

                                binding.approvalCodeEdit.doAfterTextChanged { s ->
                                    val input = s?.toString()?.trim() ?: ""
                                    val isValid = com.refayatul.fityah.utils.ApprovalUtils.verifyPin(input, pinHash)
                                    setPrimaryUnlockActionReady(isValid)
                                }
        ...
        binding.btnProceed.setOnClickListener {
            ...
            if (warningScreenConfig.isQrUnlockRequirementEnabled && !isQrScanned) {
                ...
                options.setCaptureActivity(com.refayatul.fityah.ui.activity.PortraitCaptureActivity::class.java)
            ...
            if (warningScreenConfig.isIntentRequirementEnabled) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    val log = com.refayatul.fityah.data.db.IntentLogEntity(
                        timestamp = System.currentTimeMillis(),
                        packageName = pkg,
                        intentText = intentText,
                        unlockedDurationMs = time
                    )
                    com.refayatul.fityah.data.db.AppDatabase.getInstance(this@WarningActivity).intentLogDao().insert(log)
                }
