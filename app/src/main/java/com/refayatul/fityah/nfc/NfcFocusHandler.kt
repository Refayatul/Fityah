 ...
 *   fityah://focus/<action>[?group=<groupId>&mins=<minutes>]
...
object NfcFocusHandler {
    ...
    /**
     * Parses a `fityah://focus/<action>[?group=&mins=]` URI into a [Request], or null if it isn't a
     * Fityah focus URI.
     */
    ...
    private fun isFocusActive(settings: com.refayatul.fityah.data.models.Settings): Boolean {
        val (id, endTime) = settings.activeManualFocusGroupId
        return id != null && endTime > System.currentTimeMillis()
    }

    private fun isNonExitableActive(settings: com.refayatul.fityah.data.models.Settings): Boolean {
    ...
    private suspend fun start(
        ...
        settings: com.refayatul.fityah.data.models.Settings,
        groupId: String?,
        minutes: Int?
    ): Result {
