...
import com.refayatul.fityah.data.db.FocusStatsEntity
...
import com.refayatul.fityah.utils.DndHelper
import com.refayatul.fityah.utils.TimeTools
import kotlinx.coroutines.flow.first

/**
 * Turns an API command into a real change in Fityah.
 *
 ...
 */
object FityahApiCommands {

    private const val TAG = "FityahApi"
    ...
    suspend fun runCommand(context: Context, commandName: String?, args: Bundle): Boolean {
        ...
        val target = args.getString(FityahApiContract.ARG_TARGET).orEmpty()
        val enable = args.getBoolean(FityahApiContract.ARG_ENABLE, true)
        val minutes = args.getInt(FityahApiContract.ARG_MINUTES, 25)
        ...
        return when (state) {
            ApiState.FOCUS_ACTIVE -> {
                ...
                mapOf(
                    ...
                    FityahApiContract.VAR_FOCUS_GROUP to name,
                    FityahApiContract.VAR_FOCUS_REMAINING to remainingMin.toString()
                )
            }
