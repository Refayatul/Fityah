...
import com.refayatul.fityah.receivers.AdminReceiver
...
object ServiceProtectionManager {
    ...
    fun restartVpnIfEnabled(context: Context) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val settings = DataStoreManager(context.applicationContext).settings.first()
                if (settings.vpnConfig.isEnabled) {
                    val intent = Intent(context.applicationContext, DnsVpnService::class.java).apply {
                        action = DnsVpnService.ACTION_START
                    }
                    context.startService(intent)
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to restart VPN on boot", e)
            }
        }
    }
    ...
    /** Adds the Fityah accessibility service back into the secure settings list, via Shizuku. */
    ...
    /**
     * The strongest tier. Makes Fityah a device owner, which survives force stop and blocks uninstall
